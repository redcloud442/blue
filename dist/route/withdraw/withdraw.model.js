...
FOR UPDATE`;
        ...
        const { alliance_olympus_earnings, alliance_referral_bounty, alliance_winning_earnings, } = amountMatch[0];
    ...
    `;
        await tx.alliance_withdrawal_request_table.create({
            data: {
                ...
                alliance_withdrawal_request_member_id: teamMemberProfile.alliance_member_id,
                alliance_withdrawal_request_withdraw_type: earnings,
                alliance_withdrawal_request_approved_by: countAllRequests[0]?.approver_id ?? null,
            },
        });
        // Update the earnings
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        await tx.alliance_transaction_table.create({
            data: {
                ...
                transaction_amount: status === "APPROVED"
                    ? updatedRequest.alliance_withdrawal_request_withdraw_amount
...
export const withdrawListExportPostModel = async (params) => {
    ...
    const offset = (page - 1) * limit;
    const orderBy = Prisma.sql `ORDER BY t.alliance_withdrawal_request_date DESC`;
    const commonConditions = [
  ...
  SELECT 
    ...
    TO_CHAR(t.alliance_withdrawal_request_date, 'FMMonth DD, YYYY') AS "Date Created",
    t.alliance_withdrawal_request_withdraw_type AS "Withdrawal Type",
    TO_CHAR(t.alliance_withdrawal_request_date_updated, 'FMMonth DD, YYYY') AS "Date Updated",
