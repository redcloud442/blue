import { claimPackagePutSchema, createPackagePostSchema, packagePostSchema, reinvestPackagePutSchema, updatePackageSchema, } from "../../schema/schema.js";
...
export const packagePostMiddleware = async (c, next) => {
    ...
    const isAllowed = await rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:package-post`, 50, "1m", c);
    if (!isAllowed) {
...
export const packagesReinvestPostMiddleware = async (c, next) => {
    const user = c.get("user");
    const response = await protectionMemberUser(user.id, prisma);
    if (response instanceof Response) {
        return response;
    ...
    const isAllowed = await rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:package-reinvest`, 10, "1m", c);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
    const { packageConnectionId, packageId, amountToReinvest } = await c.req.json();
    const validation = reinvestPackagePutSchema.safeParse({
        packageId,
        amountToReinvest,
        packageConnectionId,
    });
